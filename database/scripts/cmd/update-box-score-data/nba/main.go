...
func main() {
	...
	if err != nil {
		fatal("Failed to load configuration: %v\nPlease ensure NBA_GAME_ID is set as a database UUID in .env file or as an environment variable", err)
	}

	fmt.Println(strings.Repeat("=", 72))
	fmt.Println("NBA Box Score Data Generator")
	fmt.Println(strings.Repeat("=", 72))
	...
	pbpData, err := reader_nba.ReadNBAPlayByPlay(ctx, dbStore, cfg.NBAGameID)
	if err != nil {
	...
	existingBoxScores, err := store_nba.GetNBABoxScoresByGameID(dbStore, ctx, gameUUID)
	if err != nil {
