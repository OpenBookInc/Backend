...
func main() {
	...
	fmt.Println("NBA Available Legs - Build Player Props Mappings")
	fmt.Println(strings.Repeat("=", 72))
	...
	fmt.Println("\nQuerying NBA games for date...")
	games, err := dbStore.GetGamesByLeagueAndDateRange(ctx, "NBA", cfg.GameDate, cfg.GameDate, cfg.TimeZoneForDate)
	if err != nil {
		fatal("Failed to query NBA games: %v", err)
	}
	fmt.Printf("Found %d NBA game(s)\n", len(games))
	if len(games) == 0 {
		fmt.Println("\nNo NBA games found for this date. Nothing to do.")
		return
	}
	...
	fmt.Println("\n" + strings.Repeat("=", 72))
	fmt.Println("NBA available legs mapping completed successfully!")
	fmt.Println(strings.Repeat("=", 72))
}
